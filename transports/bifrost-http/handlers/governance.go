...
func (h *GovernanceHandler) updateTeam(ctx *fasthttp.RequestCtx) {
	...
	if err := h.configStore.ExecuteTransaction(ctx, func(tx *gorm.DB) error {
		...
		//   - explicit team-level field wins
		//   - else, if budgets were sent, coalesce from any incoming budget.CalendarAligned (back-compat)
		//   - else leave existing team.CalendarAligned untouched
		if req.CalendarAligned != nil {
			team.CalendarAligned = *req.CalendarAligned
		}
		if team.CalendarAligned {
			now := time.Now()
			if req.Budgets == nil {
			...
			if req.RateLimit == nil && team.RateLimit != nil {
				rl := team.RateLimit
				snapped := false
