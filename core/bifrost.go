...
func executeRequestWithRetries[T any](
	...
	for attempts = 0; attempts <= config.NetworkConfig.MaxRetries; attempts++ {
		...
		if keyProvider != nil && (attempts == 0 || lastWasKeyRotate) {
			if usedKeyIDs == nil {
			...
			if err != nil {
				var zero T
				// If keyProvider gave up because every key is in deadKeyIDs (401/402/403),
				// surface that as 502 Bad Gateway. Bifrost is a gateway and every upstream
				// credential it could try is broken on the provider's side — returning the
				// raw 401/403 would falsely suggest the caller's bifrost API key is bad,
				// when in fact only the bifrost→provider keys are misconfigured. The
				// individual per-key errors are preserved in the attempt trail.
				if len(deadKeyIDs) > 0 {
					statusCode := 502
		...
		if attempts > 0 {
			...
			// Backoff only when retrying the same key against the same endpoint (transient
			// 5xx / network). Skip it when we rotated to a different key — a different
			// credential gains nothing from waiting.
			if !lastWasKeyRotate {
		...
		//
		// isKeyRotate: failure is bound to this specific key/account (401/402/403/429, or a
		...
		shouldRetry := false
		isKeyRotate := (bifrostError.StatusCode != nil && keyRotateStatusCodes[*bifrostError.StatusCode]) ||
		...
		} else if (bifrostError.StatusCode != nil && retryableStatusCodes[*bifrostError.StatusCode]) || isKeyRotate {
			shouldRetry = true
		...
		if isKeyRotate && keyProvider != nil {
			isPermanent := bifrostError.StatusCode != nil &&
			...
			if req.RequestType == schemas.ListModelsRequest {
				...
				if isMultiKeyBatchOp || isMultiKeyFileOp || isMultiKeyContainerOp || isMultiKeyCachedContentOp {
					...
					if len(supportedKeys) == 0 {
						...
						// return the same key regardless of exclusion sets.
						...
						keyProvider = func(usedKeyIDs, deadKeyIDs map[string]bool) (schemas.Key, error) {
							...
							if len(available) == 0 {
								...
								if len(available) == 0 {
									return schemas.Key{}, fmt.Errorf("all keys for provider %s failed with permanent per-key errors (401/402/403)", provKey)
								}
